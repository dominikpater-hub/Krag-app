...
import { knownFor, checkSubstance, leafletFor, LIVERPOOL } from './interactions.js';
...
export async function renderDiary() {
  ...
  fill('#d-meds', items.filter((i) => kind(i, 'med')),
    (m) => { const u = leafletFor(m.name); return `<span><span class="v">${escapeHtml(m.name)}</span> <span class="sub">${escapeHtml(m.dose || '')}${m.time ? ' · ' + escapeHtml(m.time) : ''}</span>${u ? ` <a href="${escapeHtml(u)}" target="_blank" rel="noopener" class="srclink">${t('d.leaflet')}</a>` : ''}</span>`; });
...
// —— interakcje leków (#7) ——
function ixRow(h) {
  const link = `<a href="${LIVERPOOL}" target="_blank" rel="noopener" class="srclink">${t('ix.liverpool')}</a>`;
  return `<div class="ix-item ${h.sev === 'high' ? 'hi' : 'med'}"><div>${escapeHtml(h.msg)}</div><div class="ix-adv">${link}</div></div>`;
}
...
async function runIxCheck() {
  ...
  const none = `<div class="ix-item ok">${t('ix.none')} <a href="${LIVERPOOL}" target="_blank" rel="noopener" class="srclink">${t('ix.liverpool')}</a></div>`;
  out.innerHTML = hits.length ? hits.map(ixRow).join('') : none;
}
