...
/* Uchwyt jest POCHODNĄ klucza publicznego, nie danymi osobowymi. Wydzielone z app.js,
 * bo tożsamości pokojowe (lib/roomid.js, audyt S-2) tworzą nazwy tą samą metodą. */
...
export function pseudoFrom(hash) {
  const tail = Array.from(hash.slice(2, 4)).map((b) => b.toString(16).padStart(2, '0')).join('');
  return `${ADJ[hash[0] % ADJ.length]} ${NOU[hash[1] % NOU.length]} #${tail.toUpperCase()}`;
}
