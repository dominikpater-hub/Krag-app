 ...
 * potem 272 → 293 (brakujące warstwy, hiv-0118…0138),
 * potem 293 → 301 (trener odporności i CMV, hiv-0139…0146). */
const OCZEKIWANE_FAKTY = 301;
...
console.log('\nT-7 · warstwy dopisane 2026-08-07 (hiv-0118…0138):');
/* Tematy, których baza nie miała WCALE: HIV-2, badanie lekooporności, szybki start leczenia,
 * szczegóły PrEP i PEP, konkretne liczby przy karmieniu, definicja późnego rozpoznania
 * i pierwszy blok o świecie. Każdy musi być osiągalny — inaczej fakt leży martwy. */
[['czy hiv-2 to co innego', 'leczenie'], ['badanie lekooporności', 'leczenie'],
 ['kiedy zacząć leczenie', 'leczenie'], ['od kiedy prep chroni', 'prep'],
 ['co dostanę w pep', 'pep'], ['czy mogę karmić piersią', 'ciaza'],
 ['późne rozpoznania', 'epidemiologia'], ['ile osób na świecie żyje z hiv', 'swiat'],
 ['czy ktoś jeszcze tak ma', 'swiat']]
  .forEach(([q, b]) => { const r = route(q); ok(r && r.block === b, '"' + q + '" → ' + (r && r.block) + ' (oczekiwano ' + b + ')'); });
{ /* Fakt 0121 („można zacząć tego samego dnia") nie może być zasłonięty przez stare fakty
   * o codziennym przyjmowaniu leków — to jest odpowiedź na pytanie świeżo po diagnozie. */
  const r = route('kiedy zacząć leczenie');
  ok(r && r.ids.indexOf('0121') > -1, 'szybki start leczenia osiągalny — ids=' + (r && r.ids)); }
