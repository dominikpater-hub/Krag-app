...
/* DECYZJA właściciela 2026-08-06: autorytet jest DZIEDZINOWY, nie stopniowalny raz na zawsze.
 * Media — także ogólne — są pełnoprawnym źródłem tam, gdzie są kompetentne: sprawy społeczne,
 * język, oraz relacjonowanie tego, co się wydarzyło (nowelizacja, stanowisko urzędu, dane).
 * Przy twierdzeniach KLINICZNYCH ich rola jest inna: sygnalizują, ale ugruntowanie należy do
 * badania albo wytycznych, bo referują z drugiej ręki.
 * Poniżej bloki tematyczne, w których media ogólne są u siebie w domu. */
export const SOCIAL_BLOCKS = new Set(['stygma', 'prawo', 'epidemiologia', 'prep-pl', 'leczenie-pl']);

/**
 * Poziom wynikający ze źródła albo null, gdy źródła nie sprawdziliśmy.
 * block: blok tematyczny faktu — rozstrzyga o kompetencji dziedzinowej mediów ogólnych.
 */
export function srcTier(name, block) {
  const kind = SRC_KIND[String(name || '').trim()];
  if (!kind) return null;
  if (kind === 'PRESS') return SOCIAL_BLOCKS.has(block) ? 'verified' : 'community';
  return KIND_TIER[kind] || null;
}
