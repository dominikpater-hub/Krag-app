...
test('rozpoznaje markę → tagi substancji (także generyki z rejestru)', () => {
  ...
  assert.ok(medTags(['Valhit']).has('gcv'));               // walgancyklowir (CMV) — realny lek, rozpoznany
  assert.equal(medTags(['Xyzqwerty 10 mg']).size, 0);      // faktycznie nieznany → brak fałszywej flagi
});

test('knownFor: walgancyklowir (Valhit) → interakcja z zydowudyną (mielosupresja)', () => {
  assert.ok(knownFor(['Valhit']).some((h) => h.key === 'gcv-azt'));
});
