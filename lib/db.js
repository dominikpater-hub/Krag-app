/* Krąg — warstwa składu lokalnego (IndexedDB 'krag-local' v4).
 ...
 * v4 (audyt S-2): magazyn 'roomids' — tożsamości osobne dla pokojów. Ich klucze prywatne
 * nie opuszczają urządzenia, tak samo jak klucz konta głównego.
 */
export function db() {
  return new Promise((res, rej) => {
    const r = indexedDB.open('krag-local', 4);
    r.onupgradeneeded = () => {
      const d = r.result;
      if (!d.objectStoreNames.contains('roomids')) d.createObjectStore('roomids', { keyPath: 'roomId' });
