...
export function makeClient(baseUrl) {
  ...
  return {
    ...
    // S-2: pokój ma dwie osie — widoczność w katalogu i sposób wejścia. Domyślnie otwarty.
    roomCreate: (name, visibility, entry) => req('POST', '/rooms', { auth: true, body: { name, visibility, entry } }),
    roomJoin: (id) => req('POST', '/rooms/' + encodeURIComponent(id) + '/join', { auth: true }),
    // Wejście kluczem — bez id: pokoju ukrytego kandydat nie zna.
    roomJoinWithKey: (code) => req('POST', '/rooms/join', { auth: true, body: { code } }),
    roomKeyCreate: (id, maxUses, days) => req('POST', '/rooms/' + encodeURIComponent(id) + '/keys', { auth: true, body: { maxUses, days } }),
    roomKeyList: (id) => req('GET', '/rooms/' + encodeURIComponent(id) + '/keys', { auth: true }),
    roomKeyRevoke: (id, keyId) => req('DELETE', '/rooms/' + encodeURIComponent(id) + '/keys/' + encodeURIComponent(keyId), { auth: true }),
    roomRemove: (id, pseudonym) => req('POST', '/rooms/' + encodeURIComponent(id) + '/remove', { auth: true, body: { pseudonym } }),
    ...
    // Ida Rozumie (LLM proxy): pytanie + wybrane fakty + skrócony kontekst rozmowy
    // (ciągłość). Bez tożsamości/dziennika — do modelu idzie sam tekst.
    idaAsk: (q, facts, lang, history) => req('POST', '/ida/ask', { auth: true, body: { q, facts, lang, history } }),
  };
}
