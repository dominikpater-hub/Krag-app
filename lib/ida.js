...
const JUDGE_RESULT = /czy to (zle|dobrze|dobry|zly|normalne|niepokoj)|is (that|this) (bad|good|normal|ok|okay|concerning)/;

/** 'judge' (ocena wyniku) | 'decide' (decyzja leczenia) | 'trend' (samo dzielenie się) | null. */
export function boundVariant(q) {
  const n = norm(q);
  if (JUDGE_ALWAYS.test(n)) return 'judge';
  const hasResult = RESULT.test(n);
  const hasNumber = /\bcd4 \d+|wiremia \d+/.test(n);
  if (DECIDE.test(n) && (hasResult || MED.test(n))) return 'decide';
  if (JUDGE_RESULT.test(n) && hasResult) return 'judge';
  if (hasResult && (MINE.test(n) || hasNumber)) return 'trend';
  return null;
}
...
export function confBadge(c, src) {
  const fromSrc = src ? srcTier(src) : null;
