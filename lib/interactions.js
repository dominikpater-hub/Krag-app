 ...
 */
'use strict';
import { IX_RULES, DRUG_TAGS, INSTI, LEAFLET, LIVERPOOL } from './interactions-data.js';
export { LIVERPOOL };
...
const KW = [
  ...
  [['walgancyklow', 'valganciclov', 'gancyklow', 'ganciclov'], 'gcv'],
];
...
// Treść reguły jest źródłem; nie dokładamy „potwierdź z lekarzem" — baza referencyjna
// (Liverpool) jest rzetelnym źródłem. Link do niej dokłada warstwa UI (ix-adv).
function toItem(r, lang) {
  return { key: r.a + '-' + r.b, sev: SEV[r.sev] || 'medium', msg: lang === 'en' ? r.en : r.pl };
}
