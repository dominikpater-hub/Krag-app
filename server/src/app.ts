...
export function buildApp(
  ...
  const limits = {
    ...
    /* DECYZJA właściciela 2026-08-06: limit rejestracji per IP WYŁĄCZONY (0).
     * Powód: od S-2 fazy 2 dołączenie do pokoju zakłada osobne konto (tożsamość pokojowa),
     * a operatorzy komórkowi dzielą jeden adres między wielu abonentów (CGNAT) — taki limit
     * trafiał głównie w przypadkowe osoby, a nie w napastnika (ten ma pulę adresów i VPN).
     * Zaporą zostaje DOWÓD PRACY przy zakładaniu konta oraz limity per KONTO na /ida/ask
     * (koszt modelu), /rooms/:id/join i /keys/:pseudonym.
     * Włączenie z powrotem: KRAG_RL_REGISTER_IP=30 — bez zmian w kodzie. */
    registerIp: createLimiter({ windowMs: 60 * MIN, max: L.registerIp ?? envInt('KRAG_RL_REGISTER_IP', 0) }),
